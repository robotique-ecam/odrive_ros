...
class ODriveNode(Node):
    ...
    def loop_cb(self):
        ...
        #    self.get_logger().error('interrupt exception')
    ...
    def fast_timer_cb(self):
        ...
        try:
            ...
            self.get_logger().error('failed to pub')
        ...
        if self.fast_timer_comms_active and not self.command_queue.empty():
            self.get_logger().info('have a command to exec')
            # check to see if we're initialised and engaged motor
            # try:
            #    if not self.driver.has_prerolled(): #ensure_prerolled():
            #        self.get_logger().warn('ODrive has not been prerolled, ignoring drive command.')
            ...
            if motor_command[0] == 'drive':
                try:
                    self.get_logger().info('trying a drive command')
                    ...
                    self.get_logger().error('motor drive unknown failure: {0}'.format(traceback.format_exc()))
                    self.fast_timer_comms_active = False
    ...
    def terminate(self):
        self.get_logger().info('attempting to terminate')
        try:
            if self.fast_timer:
            ...
            self.get_logger().warn('failed to terminate')
            raise exc
    ...
    def connect_driver(self, request: std_srvs.srv.Trigger.Request, resp: std_srvs.srv.Trigger.Response) -> std_srvs.srv.Trigger.Response:
        self.get_logger().info('attempting to connect driver')

        if self.driver:
            resp.success = False
            resp.message = 'Already connected.'
            self.get_logger().warn('already connected')
            return resp
        try:
            ODriveClass = ODriveInterfaceAPI if not self.sim_mode else ODriveInterfaceSimulator

            self.driver = ODriveClass(logger=self.get_logger())
    ...
    def disconnect_driver(self, request: std_srvs.srv.Trigger.Request, resp: std_srvs.srv.Trigger.Response) -> std_srvs.srv.Trigger.Response:
        self.get_logger().info('attempting to disconnect driver')
    ...
    def calibrate_motor(self, request: std_srvs.srv.Trigger.Request, resp: std_srvs.srv.Trigger.Response) -> std_srvs.srv.Trigger.Response:
        self.get_logger().info('attempting to calibrate motor')

        if not self.driver:
            self.get_logger().error('Not conected.')
            resp.success = False
            resp.message = 'Not connected.'
    ...
    def cmd_vel_callback(self, msg):
        ...
        self.get_logger().info(f'Linear Components: [{msg.linear.x}, {msg.linear.y}, {msg.linear.z}]')
        self.get_logger().info(f'Angular Components: [{msg.angular.x}, {msg.angular.y}, {msg.angular.z}]')
